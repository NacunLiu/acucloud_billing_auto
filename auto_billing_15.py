...
# unittest的核心：TestCase(测试用例) TestSuite(测试套件) TestLoader(测试加载) TestRunner(测试执行)/TestTextRunner(文本方式执行测试用例) Fixture(测试夹具)
# 参数化：为了解决相同操作不同数据的问题，避免代码的冗余 根据需求动态获取参数并引用的过程就是参数化
# unittest并不带参数化,我们需要单独的组件parameterized进行参数化
